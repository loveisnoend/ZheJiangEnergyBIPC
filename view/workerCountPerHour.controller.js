sap.ui.controller("com.zhenergy.pcbi.view.workerCountPerHour", {
	...
	loadBase_SupplyWorkerCountPerHourIncome: function(chartDivId, priceChartName) {
		...
		mParameters['success'] = jQuery.proxy(function(sRes) {
			...
			// 			for (var i in sRes.results) {
			// 				// 发电生产率收入同比
			// 				if (sRes.results[i].KPI_TYPE == '发电生产率_同比') {
...
// 			$('#workerCountPerHourIncomeStatisticDate').html(dataStatisticDate);
		...
		sap.ui.getCore().getModel().read("AT_ZSCREEN_FZBZ_02_V03/?$filter=(BNAME eq '" + usrid + "')", mParameters);
	},
	// 获取个电厂指标-发电生产率 SCREEN_FZBZ_02_V02
	loadEachPlant_SupplyWorkerCountPerHourIncome: function(chartDivId, priceChartName, powerPlantName) {
		...
		mParameters['success'] = jQuery.proxy(function(sRes) {
			...
			for (var i in sRes.results) {
				// 发电生产率收入同比
				if (sRes.results[i].KPI_TYPE == '发电生产率同比') {
					KPI_DLR_UP.push(sRes.results[i].KPI_VALUE);
				}
				// 发电生产率收入
				if (sRes.results[i].KPI_TYPE == '发电生产率' && sRes.results[i].KPI_DESC == powerPlantName) {
...
// 			$('#workerCountPerHourIncomeStatisticDate').html(dataStatisticDate);
	...
	loadBaseDataDetail_WorkerCountPerHourIncome: function(chartDivId, priceChartName, xData, KPI_DLR_V, KPI_DLR_UP) {
		...
		function draw(e) {
			...
			// 			document.getElementById('profitNameWorkerCountPerHour').innerHTML = document.getElementById(
			// 				'powerPlantMainDetailTitleWorkerCountPerHour').innerHTML;
			var color1 = '#A704CA';
			var color2 = '#E52DE6';
			var option = {
				title: {
...
// 			myChart3.setOption(option3);
			...
			///////////////////////////////新疆阿克苏地图////////////////////////////////////////////
			// 基于准备好的dom，初始化echarts图表
			myChart6 = ec.init(document.getElementById('akesuMapWorkerCountPerHour'));
			...
			///////////////////////////////宁夏枣泉地图////////////////////////////////////////////
			// 基于准备好的dom，初始化echarts图表
			myChart7 = ec.init(document.getElementById('zaoquanMapWorkerCountPerHour'));
