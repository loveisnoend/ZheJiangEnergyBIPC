sap.ui.controller("com.zhenergy.pcbi.view.businessProfit", {
	...
	onInit: function() {
		this.getView().addEventDelegate({
			...
			onAfterShow: jQuery.proxy(function(evt) {
				...
				sap.ui.controller("com.zhenergy.pcbi.view.templates.dymcontents").onInsertMap(document,"BusinessProfit");
			    //AC-LOUWW 页面增加动态的时间日期标签
				var myDate=new Date() ;
				var timeLabel = myDate.getFullYear() + "年" + myDate.getMonth() +"月"; //getMonth 1-12月对应0-11
				var naviDemo = document.getElementById("naviBusinessProfit");
		        naviDemo.innerHTML =  "<span id='demo' style='height:100%;'>"+
                                "<b onClick='toPureProfitPage()' style='cursor:pointer;'>浙能电力净利润</b> > <b>"+timeLabel+"营业利润</b>"+
                                "</span>";
	...
	onAfterShow: function() {
		...
		document.getElementById('internetDetailBusinessProfit').style.display = "";
		document.getElementById('rlcb_detailBusinessProfit').style.display = "none";
		// this.loadChart();
		this._loadData01();
		// 设定头部跑马灯信息 common.js
	...
	// 获取集团指标-营业利润 SCREEN_ZCQK_02_V01
	loadBase_SupplyBusinessProfitIncome: function(chartDivId, priceChartName) {
		...
		mParameters['success'] = jQuery.proxy(function(sRes) {
			...
			for (var i in sRes.results) {
				// 营业利润同比
				if (sRes.results[i].KPI_TYPE == '营业利润_同比') {
					KPI_JZC_UP.push(sRes.results[i].KPI_VALUE);
				}
				// 营业利润
				if (sRes.results[i].KPI_TYPE == '营业利润' && sRes.results[i].KPI_DATE == sRes.results[sRes.results.length - 1].KPI_DATE) {
			...
			// 统计于日期
			$('#businessProfitIncomeStatisticDate').html(dataStatisticDate);
			if (priceChartName == '营业利润') {
				this.businessProfit(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP);
			}
			if (busy) {
				busy.close();
			}
		...
		sap.ui.getCore().getModel().read("SCREEN_JYYJ_03_YYLR/?$filter=(BNAME eq '" + usrid + "')", mParameters);
	},
	// 获取个电厂指标-营业利润 SCREEN_ZCQK_02_V01
	loadEachPlant_SupplyBusinessProfitIncome: function(chartDivId, priceChartName, powerPlantName) {
		...
		mParameters['success'] = jQuery.proxy(function(sRes) {
			...
			for (var i in sRes.results) {
				...
				// 营业利润
				if (sRes.results[i].KPI_TYPE == '营业利润' && sRes.results[i].KPI_DESC == powerPlantName) {
					KPI_JZC_V.push(sRes.results[i].KPI_VALUE);
			...
			$('#businessProfitIncomeStatisticDate').html(dataStatisticDate);
			if (priceChartName == '营业利润') {
				this.loadBaseDataDetail_BusinessProfitIncome(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP);
			}
			if (busy) {
				busy.close();
			}
	...
	// 加载集团-营业利润
	businessProfit: function(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP) {
		...
		function draw(e) {
			var mychart = e.init(document.getElementById(chartDivId));
					if(document.getElementById('powerPlantMainDetailTitleBusinessProfit')
.innerHTML=="集团"){
   document.getElementById('profitNameBusinessProfit').innerHTML="电力股份公司";
}else{
document.getElementById('profitNameBusinessProfit').innerHTML = document.getElementById('powerPlantMainDetailTitleBusinessProfit')
.innerHTML;
}
//			document.getElementById('profitNameBusinessProfit').innerHTML = document.getElementById('powerPlantMainDetailTitleBusinessProfit').innerHTML;
			var color1 = '#A704CA';
			var color2 = '#E52DE6';
			var option = {
				title: {
	...
	// 加载集团-营业利润指标
	loadBaseDataDetail_BusinessProfitIncome: function(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP) {
		require(
            [
                'echarts',
                'echarts/chart/line',
...
.innerHTML;
	...
	loadChart: function(map1Data, map2Data, map3Data, map4Data) {
		...
		function draw(e) {
			drawBusinessProfitDistribution(e);
		...
		function drawBusinessProfitDistribution(ec) {
...
// 			myChart3 = ec.init(document.getElementById('chinaMapBusinessProfit'));
// 			option3 = {
// 				tooltip: {
// 					trigger: 'item',
// 					formatter: '{b}'
...
// 			myChart3.setOption(option3);
			...
			document.getElementById('powerPlantMainDetailTitleBusinessProfit').innerHTML = '集团'
			//////////////////////////////////浙江省地图//////////////////////////////////////////////////////////		
			// 基于准备好的dom，初始化echarts图表
			var myChart4 = ec.init(document.getElementById('powerPlantMapBusinessProfit'));
			...
			///////////////////////////////安徽淮南市地图////////////////////////////////////////////
			// 基于准备好的dom，初始化echarts图表
			myChart5 = ec.init(document.getElementById('huaiNanMapBusinessProfit'));
			...
			///////////////////////////////新疆阿克苏地图////////////////////////////////////////////
			// 基于准备好的dom，初始化echarts图表
			myChart6 = ec.init(document.getElementById('akesuMapBusinessProfit'));
			...
			///////////////////////////////宁夏枣泉地图////////////////////////////////////////////
			// 基于准备好的dom，初始化echarts图表
			myChart7 = ec.init(document.getElementById('zaoquanMapBusinessProfit'));
		...
		function drawpie01(e) {
			drawpie(e, 3, 4, 'detail_pieBusinessProfit');
		}

		function drawbar01(e) {
			drawbar(e, 4, 6, 'detail_01BusinessProfit');
		}

		function drawbar02(e) {
			drawbar(e, 7, 3, 'detail_02BusinessProfit');
		}

		function drawbar03(e) {
			drawbar(e, 3, 7, 'detail_03BusinessProfit');
		}

		function drawbar04(e) {
			drawbar(e, 8, 2, 'detail_04BusinessProfit');
		}
		// 设置Chart的数据
		function setChartData(ec, mapSeries, dataIndex) {
			...
			document.getElementById('powerPlantMainDetailTitleBusinessProfit').innerHTML = powerPlantName;

			var priceChartId = "priceDetailDivBusinessProfit";
			var priceChartName = "营业利润";
			...
			if (powerPlantName == '集团') {
				// TODO
				businessProfit.getController().loadBase_SupplyBusinessProfitIncome(priceChartId, priceChartName);
				//businessProfit.getController().loadEachPlant_SupplyBusinessProfitIncome(priceChartId, priceChartName, powerPlantName);
			} else {
				businessProfit.getController().loadEachPlant_SupplyBusinessProfitIncome(priceChartId, priceChartName, powerPlantName);
			}
			//  // 自产蒸汽
			...
			//      document.getElementById('travelPriceBusinessProfit').innerHTML =  selfSteamIncomeVal;
			//  } else {
			//      document.getElementById('travelPriceBusinessProfit').innerHTML = 0;
			...
			//      document.getElementById('coalPriceBusinessProfit').innerHTML = outSteamIncomeVal;
			//  } else {
			//      document.getElementById('coalPriceBusinessProfit').innerHTML = 0;
			//      outSteamIncomeVal = 0;
			//  }
			//  // 热水
			//  var businessProfitWaterIncomeVal = mapSeries.markPoint.data[dataIndex].businessProfitWaterIncomeVal;
			//  if (businessProfitWaterIncomeVal != undefined) {
			//      document.getElementById('watt1BusinessProfit').innerHTML =  businessProfitWaterIncomeVal;
			//  } else {
			//      document.getElementById('watt1BusinessProfit').innerHTML = 0;
			//      businessProfitWaterIncomeVal = 0;
			//  }
			//  // 初装费
			...
			//  }
			//  // 供热收入
			//  var supplyBusinessProfitIncomeVal = mapSeries.markPoint.data[dataIndex].supplyBusinessProfitIncomeVal;
			//  if (supplyBusinessProfitIncomeVal != undefined) {
			//      document.getElementById('fuelCostBusinessProfit').innerHTML = supplyBusinessProfitIncomeVal;
			//  } else {
			//      document.getElementById('fuelCostBusinessProfit').innerHTML = 0;
			//      supplyBusinessProfitIncomeVal = 0;
			//  }
			//  // 供热收入同比
			//  var supplyBusinessProfitIncomeUP = mapSeries.markPoint.data[dataIndex].supplyBusinessProfitIncomeUP;
			//  if (supplyBusinessProfitIncomeUP != undefined) {
			//      document.getElementById('fuelDownPercentBusinessProfit').innerHTML = supplyBusinessProfitIncomeUP;
			//  } else {
			//      document.getElementById('fuelDownPercentBusinessProfit').innerHTML = 0;
			//      supplyBusinessProfitIncomeUP = 0;
			//  }
			//  var dataAll = selfSteamIncomeVal + outSteamIncomeVal + businessProfitWaterIncomeVal + firstFeeIncomeVal;
			//  if (dataAll == 0) {
			//      dataAll = 10;
			//  }
			//  drawpie(ec, supplyBusinessProfitIncomeUP+50, 50, 'detail_pieBusinessProfit');
			//  drawbar(ec, selfSteamIncomeVal, dataAll, 'detail_01BusinessProfit');
			//  drawbar(ec, outSteamIncomeVal, dataAll, 'detail_02BusinessProfit');
			//  drawbar(ec, businessProfitWaterIncomeVal, dataAll, 'detail_03BusinessProfit');
			//  drawbar(ec, firstFeeIncomeVal, dataAll, 'detail_04BusinessProfit');
		}
	}
});