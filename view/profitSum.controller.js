sap.ui.controller("com.zhenergy.pcbi.view.profitSum", {
	...
	onAfterShow: function() {
		...
		document.getElementById('internetDetailProfitSum').style.display = "";
		document.getElementById('rlcb_detailProfitSum').style.display = "none";
		// this.loadChart();
		this._loadData01();
		// 设定头部跑马灯信息 common.js
	...
	// 获取浙能电力指标-利润总额 SCREEN_ZCQK_02_V01
	loadBase_SupplyProfitSumIncome: function(chartDivId, priceChartName) {
		...
		mParameters['success'] = jQuery.proxy(function(sRes) {
			...
			for (var i in sRes.results) {
			    if (sRes.results[i].KPI_DESC != '浙能电力本部') {
    				// 利润总额同比
    				if (sRes.results[i].KPI_TYPE == '利润总额_同比') {
    					KPI_JZC_UP.push(sRes.results[i].KPI_VALUE);
    				}
    				// 利润总额
    				if (sRes.results[i].KPI_TYPE == '利润总额' && sRes.results[i].KPI_DATE == sRes.results[sRes.results.length - 1].KPI_DATE) {
			...
			// 统计于日期
// 			$('#profitSumIncomeStatisticDate').html(dataStatisticDate);
			if (priceChartName == '利润总额') {
				this.profitSum(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP);
			}
			if (busy) {
				busy.close();
			}
	...
	loadEachPlant_SupplyProfitSumIncome: function(chartDivId, priceChartName, powerPlantName) {
		...
		mParameters['success'] = jQuery.proxy(function(sRes) {
			...
			for (var i in sRes.results) {
				...
				// 利润总额
				if (sRes.results[i].KPI_TYPE == '利润总额' && sRes.results[i].KPI_DESC == powerPlantName) {
					KPI_JZC_V.push(sRes.results[i].KPI_VALUE);
...
// 			$('#profitSumIncomeStatisticDate').html(dataStatisticDate);
			if (priceChartName == '利润总额') {
				this.loadBaseDataDetail_ProfitSumIncome(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP);
			}
			if (busy) {
				busy.close();
			}
	...
	profitSum: function(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP) {
		...
		function draw(e) {
			...
			if(document.getElementById('powerPlantMainDetailTitleProfitSum')
.innerHTML=="浙能电力"){
   document.getElementById('profitNameProfitSum').innerHTML="浙能电力股份有限公司";
}else{
document.getElementById('profitNameProfitSum').innerHTML = document.getElementById('powerPlantMainDetailTitleProfitSum')
.innerHTML;
}
// 			document.getElementById('profitNameProfitSum').innerHTML = document.getElementById('powerPlantMainDetailTitleProfitSum').innerHTML;
			var color1 = '#A704CA';
			var color2 = '#E52DE6';
			var option = {
				title: {
	...
	// 加载浙能电力-利润总额指标
	loadBaseDataDetail_ProfitSumIncome: function(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP) {
		require(
            [
                'echarts',
                'echarts/chart/line',
...
// 			document.getElementById('profitNameProfitSum').innerHTML = document.getElementById('powerPlantMainDetailTitleProfitSum').innerHTML;
	...
	loadChart: function(map1Data, map2Data, map3Data, map4Data) {
		...
		function drawProfitSumDistribution(ec) {
...
// 			myChart3 = ec.init(document.getElementById('chinaMapProfitSum'));
// 			option3 = {
// 				tooltip: {
// 					trigger: 'item',
// 					formatter: '{b}'
...
// 			myChart3.setOption(option3);
			...
			document.getElementById('powerPlantMainDetailTitleProfitSum').innerHTML = '浙能电力股份有限公司'
			//////////////////////////////////浙江省地图//////////////////////////////////////////////////////////		
			// 基于准备好的dom，初始化echarts图表
			myChart4 = ec.init(document.getElementById('powerPlantMapProfitSum'));
			...
			///////////////////////////////安徽淮南市地图////////////////////////////////////////////
			// 基于准备好的dom，初始化echarts图表
			myChart5 = ec.init(document.getElementById('huaiNanMapProfitSum'));
			...
			///////////////////////////////新疆阿克苏地图////////////////////////////////////////////
			// 基于准备好的dom，初始化echarts图表
			myChart6 = ec.init(document.getElementById('akesuMapProfitSum'));
			...
			///////////////////////////////宁夏枣泉地图////////////////////////////////////////////
			// 基于准备好的dom，初始化echarts图表
			myChart7 = ec.init(document.getElementById('zaoquanMapProfitSum'));
		...
		function drawpie01(e) {
			drawpie(e, 3, 4, 'detail_pieProfitSum');
		}

		function drawbar01(e) {
			drawbar(e, 4, 6, 'detail_01ProfitSum');
		}

		function drawbar02(e) {
			drawbar(e, 7, 3, 'detail_02ProfitSum');
		}

		function drawbar03(e) {
			drawbar(e, 3, 7, 'detail_03ProfitSum');
		}

		function drawbar04(e) {
			drawbar(e, 8, 2, 'detail_04ProfitSum');
		}
		// 设置Chart的数据
		function setChartData(ec, mapSeries, dataIndex) {
			...
			document.getElementById('powerPlantMainDetailTitleProfitSum').innerHTML = powerPlantName;

			var priceChartId = "priceDetailDivProfitSum";
			var priceChartName = "利润总额";
			...
			if (powerPlantName == '浙能电力') {
				// TODO
				profitSum.getController().loadBase_SupplyProfitSumIncome(priceChartId, priceChartName);
				//profitSum.getController().loadEachPlant_SupplyProfitSumIncome(priceChartId, priceChartName, powerPlantName);
			} else {
				profitSum.getController().loadEachPlant_SupplyProfitSumIncome(priceChartId, priceChartName, powerPlantName);
			}
			//  // 自产蒸汽
			...
			//      document.getElementById('travelPriceProfitSum').innerHTML =  selfSteamIncomeVal;
			//  } else {
			//      document.getElementById('travelPriceProfitSum').innerHTML = 0;
			...
			//      document.getElementById('coalPriceProfitSum').innerHTML = outSteamIncomeVal;
			//  } else {
			//      document.getElementById('coalPriceProfitSum').innerHTML = 0;
			//      outSteamIncomeVal = 0;
			//  }
			//  // 热水
			//  var profitSumWaterIncomeVal = mapSeries.markPoint.data[dataIndex].profitSumWaterIncomeVal;
			//  if (profitSumWaterIncomeVal != undefined) {
			//      document.getElementById('watt1ProfitSum').innerHTML =  profitSumWaterIncomeVal;
			//  } else {
			//      document.getElementById('watt1ProfitSum').innerHTML = 0;
			//      profitSumWaterIncomeVal = 0;
			//  }
			//  // 初装费
			...
			//  }
			//  // 供热收入
			//  var supplyProfitSumIncomeVal = mapSeries.markPoint.data[dataIndex].supplyProfitSumIncomeVal;
			//  if (supplyProfitSumIncomeVal != undefined) {
			//      document.getElementById('fuelCostProfitSum').innerHTML = supplyProfitSumIncomeVal;
			//  } else {
			//      document.getElementById('fuelCostProfitSum').innerHTML = 0;
			//      supplyProfitSumIncomeVal = 0;
			//  }
			//  // 供热收入同比
			//  var supplyProfitSumIncomeUP = mapSeries.markPoint.data[dataIndex].supplyProfitSumIncomeUP;
			//  if (supplyProfitSumIncomeUP != undefined) {
			//      document.getElementById('fuelDownPercentProfitSum').innerHTML = supplyProfitSumIncomeUP;
			//  } else {
			//      document.getElementById('fuelDownPercentProfitSum').innerHTML = 0;
			//      supplyProfitSumIncomeUP = 0;
			//  }
			//  var dataAll = selfSteamIncomeVal + outSteamIncomeVal + profitSumWaterIncomeVal + firstFeeIncomeVal;
			//  if (dataAll == 0) {
			//      dataAll = 10;
			//  }
			//  drawpie(ec, supplyProfitSumIncomeUP+50, 50, 'detail_pieProfitSum');
			//  drawbar(ec, selfSteamIncomeVal, dataAll, 'detail_01ProfitSum');
			//  drawbar(ec, outSteamIncomeVal, dataAll, 'detail_02ProfitSum');
			//  drawbar(ec, profitSumWaterIncomeVal, dataAll, 'detail_03ProfitSum');
			//  drawbar(ec, firstFeeIncomeVal, dataAll, 'detail_04ProfitSum');
		}
	}
});