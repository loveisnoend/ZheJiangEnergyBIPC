sap.ui.controller("com.zhenergy.pcbi.view.businessSumIncome", {
	...
	onInit: function() {
		this.getView().addEventDelegate({
			...
			onAfterShow: jQuery.proxy(function(evt) {
				...
				sap.ui.controller("com.zhenergy.pcbi.view.templates.dymcontents").onInsertMap(document,"BusinessSumIncome");
			    //AC-LOUWW 页面增加动态的时间日期标签
				var myDate=new Date() ;
				var timeLabel = myDate.getFullYear() + "年" + myDate.getMonth() +"月"; //getMonth 1-12月对应0-11
				var naviDemo = document.getElementById("naviBusinessSumIncome");
	...
	// 获取集团指标-营业总收入 SCREEN_ZCQK_02_V01
	loadBase_SupplyBusinessSumIncomeIncome: function(chartDivId, priceChartName) {
		...
		mParameters['success'] = jQuery.proxy(function(sRes) {
			...
			for (var i in sRes.results) {
				// 营业总收入同比
				if (sRes.results[i].KPI_TYPE == '营业总收入_同比') {
					KPI_JZC_UP.push(sRes.results[i].KPI_VALUE);
				}
				// 营业总收入
				if (sRes.results[i].KPI_TYPE == '营业总收入' && sRes.results[i].KPI_DATE == sRes.results[sRes.results.length - 1].KPI_DATE) {
			...
			// 统计于日期
			$('#businessSumIncomeIncomeStatisticDate').html(dataStatisticDate);
			if (priceChartName == '营业总收入') {
				this.businessSumIncome(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP);
			}
			if (busy) {
				busy.close();
			}
	...
	// 获取个电厂指标-营业总收入 SCREEN_ZCQK_02_V01
	loadEachPlant_SupplyBusinessSumIncomeIncome: function(chartDivId, priceChartName, powerPlantName) {
		...
		mParameters['success'] = jQuery.proxy(function(sRes) {
			...
			for (var i in sRes.results) {
				...
				// 营业总收入
				if (sRes.results[i].KPI_TYPE == '营业总收入' && sRes.results[i].KPI_DESC == powerPlantName) {
					KPI_JZC_V.push(sRes.results[i].KPI_VALUE);
			...
			$('#businessSumIncomeIncomeStatisticDate').html(dataStatisticDate);
			if (priceChartName == '营业总收入') {
				this.loadBaseDataDetail_BusinessSumIncomeIncome(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP);
			}
			if (busy) {
				busy.close();
			}
	...
	// 加载集团-营业总收入
	businessSumIncome: function(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP) {
...
.innerHTML=="集团"){
   document.getElementById('profitNameBusinessSumIncome').innerHTML="电力股份公司";
}else{
...
// 				.innerHTML;
	...
	// 加载集团-营业总收入指标
	loadBaseDataDetail_BusinessSumIncomeIncome: function(chartDivId, priceChartName, xData, KPI_JZC_V, KPI_JZC_UP) {
		require(
            [
                'echarts',
                'echarts/chart/line',
...
// 				.innerHTML;
	...
	loadChart: function(map1Data, map2Data, map3Data, map4Data) {
		...
		function draw(e) {
			drawBusinessSumIncomeDistribution(e);
...
// 			myChart3.setOption(option3);
		...
		function drawpie01(e) {
			drawpie(e, 3, 4, 'detail_pieBusinessSumIncome');
		}

		function drawbar01(e) {
			drawbar(e, 4, 6, 'detail_01BusinessSumIncome');
		}

		function drawbar02(e) {
			drawbar(e, 7, 3, 'detail_02BusinessSumIncome');
		}

		function drawbar03(e) {
			drawbar(e, 3, 7, 'detail_03BusinessSumIncome');
		}

		function drawbar04(e) {
			drawbar(e, 8, 2, 'detail_04BusinessSumIncome');
		}
		// 设置Chart的数据
		function setChartData(ec, mapSeries, dataIndex) {
			...
			if (powerPlantName == '集团') {
				// TODO
				businessSumIncome.getController().loadBase_SupplyBusinessSumIncomeIncome(priceChartId, priceChartName);
				//businessSumIncome.getController().loadEachPlant_SupplyBusinessSumIncomeIncome(priceChartId, priceChartName, powerPlantName);
			} else {
				businessSumIncome.getController().loadEachPlant_SupplyBusinessSumIncomeIncome(priceChartId, priceChartName, powerPlantName);
			}
			//  // 自产蒸汽
			...
			//      document.getElementById('travelPriceBusinessSumIncome').innerHTML =  selfSteamIncomeVal;
			//  } else {
			//      document.getElementById('travelPriceBusinessSumIncome').innerHTML = 0;
			...
			//      document.getElementById('coalPriceBusinessSumIncome').innerHTML = outSteamIncomeVal;
			//  } else {
			//      document.getElementById('coalPriceBusinessSumIncome').innerHTML = 0;
			//      outSteamIncomeVal = 0;
			//  }
			//  // 热水
			//  var businessSumIncomeWaterIncomeVal = mapSeries.markPoint.data[dataIndex].businessSumIncomeWaterIncomeVal;
			//  if (businessSumIncomeWaterIncomeVal != undefined) {
			//      document.getElementById('watt1BusinessSumIncome').innerHTML =  businessSumIncomeWaterIncomeVal;
			//  } else {
			//      document.getElementById('watt1BusinessSumIncome').innerHTML = 0;
			//      businessSumIncomeWaterIncomeVal = 0;
			//  }
			//  // 初装费
			...
			//  }
			//  // 供热收入
			//  var supplyBusinessSumIncomeIncomeVal = mapSeries.markPoint.data[dataIndex].supplyBusinessSumIncomeIncomeVal;
			//  if (supplyBusinessSumIncomeIncomeVal != undefined) {
			//      document.getElementById('fuelCostBusinessSumIncome').innerHTML = supplyBusinessSumIncomeIncomeVal;
			//  } else {
			//      document.getElementById('fuelCostBusinessSumIncome').innerHTML = 0;
			//      supplyBusinessSumIncomeIncomeVal = 0;
			//  }
			//  // 供热收入同比
			//  var supplyBusinessSumIncomeIncomeUP = mapSeries.markPoint.data[dataIndex].supplyBusinessSumIncomeIncomeUP;
			//  if (supplyBusinessSumIncomeIncomeUP != undefined) {
			//      document.getElementById('fuelDownPercentBusinessSumIncome').innerHTML = supplyBusinessSumIncomeIncomeUP;
			//  } else {
			//      document.getElementById('fuelDownPercentBusinessSumIncome').innerHTML = 0;
			//      supplyBusinessSumIncomeIncomeUP = 0;
			//  }
			//  var dataAll = selfSteamIncomeVal + outSteamIncomeVal + businessSumIncomeWaterIncomeVal + firstFeeIncomeVal;
			//  if (dataAll == 0) {
			//      dataAll = 10;
			//  }
			//  drawpie(ec, supplyBusinessSumIncomeIncomeUP+50, 50, 'detail_pieBusinessSumIncome');
			//  drawbar(ec, selfSteamIncomeVal, dataAll, 'detail_01BusinessSumIncome');
			//  drawbar(ec, outSteamIncomeVal, dataAll, 'detail_02BusinessSumIncome');
			//  drawbar(ec, businessSumIncomeWaterIncomeVal, dataAll, 'detail_03BusinessSumIncome');
			//  drawbar(ec, firstFeeIncomeVal, dataAll, 'detail_04BusinessSumIncome');
		}
	}
});